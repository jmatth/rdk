...
type Client struct {
	...
	writer *protoutils.DelimitedProtoWriter
}
...
func NewExporterForPath(path, filename string) (*otlptrace.Exporter, error) {
	client, err := NewClient(path, filename)
	if err != nil {
		return nil, err
	}
...
func NewClient(dirPath, filename string) (*Client, error) {
	...
	writer := protoutils.NewDelimitedProtoWriter(logger)
	return &Client{
		logger: logger,
		writer: writer,
	}, nil
